...
def greedy_coloring_BFS(graph: Graph) -> list[int]:
    visited = set()
    colors = {}
    color_result = []
    ...
    while container:
        ...
        used_colors = set()
        for neighbor_id, _ in graph.neighbours(current_id):
        ...
        for neighbor_id, _ in graph.neighbours(current_id):
            if neighbor_id not in visited and neighbor_id not in container:
                container.append(neighbor_id)

    for vertex_id in graph.vertices():
        color_result.append((graph.get_vertex(vertex_id).key, colors[vertex_id]))

    return color_result


def greedy_coloring_DFS(graph: Graph) -> list[int]:
    visited = set()
    colors = {}
    color_result = []
...
if __name__ == '__main__':
    ...
    if option == '1':
        coloring_bfs = greedy_coloring_BFS(graph)
    elif option == '2':
        coloring_bfs = greedy_coloring_DFS(graph)
    else:
        raise Exception("Invalid option")

    polska.draw_map(graph, coloring_bfs)
